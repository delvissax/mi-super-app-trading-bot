...
class FunctionIntentAnalyzer {
  ...
  calculateComplexity(code) {
    ...
    return {
      ...
      maxNesting,
      recommendation: score > 20 ? 'Consider breaking into smaller functions' : 'Complexity is acceptable',
    };
  }
  ...
  generateRecommendation(intentions, complexity, features) {
    ...
    if (complexity.score > 20) {
      recommendations.push('⚠️ Alta complejidad: considerar refactorizar en funciones más pequeñas');
    }
...
class CodeAutoFixer {
  ...
  fixWebSocketIssues(code) {
    ...
    while ((match = sendPattern.exec(code)) !== null) {
      ...
      if (!contextBefore.includes(`${varName}.readyState`) &&
    ...
    replacements.reverse().forEach(repl => {
      ...
      if (repl.needsClosing) {
        ...
        if (closeIndex !== -1) {
          afterModified = after.substring(0, closeIndex + 1) + ' }' + after.substring(closeIndex + 1);
        }
      }
