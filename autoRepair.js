...
class AutoRepairLetal {
  ...
  checkDuplicateFunctions() {
    const functionPattern =
      /(?:function\s+(\w+)|const\s+(\w+)\s*=\s*(?:async\s*)?\(|let\s+(\w+)\s*=\s*(?:async\s*)?\()/g;
    ...
    const duplicates = Array.from(functions.entries()).filter(
      ([name, occurrences]) => occurrences.length > 1
    );
  ...
  checkUndefinedVariables() {
    ...
    requiredGlobals.forEach((varName) => {
      ...
      const definitionPattern = new RegExp(
        `(const|let|var|global\\.${varName}|function)\\s+${varName}`,
        "g"
      );
  ...
  async runAutoMode() {
    ...
    if (failCount > 0) {
      console.log(
        this.colorize(
          "⚠️  Algunos fixes fallaron. Revisa el reporte para más detalles.\n",
          "yellow"
        )
      );
    }
  ...
  async fixDuplicateFunction(name, occurrences, choice) {
    ...
    console.log(
      this.colorize(
        `\n      ✅ Eliminados ${occurrences.length - 1} duplicado(s) de ${name}()`,
        "green"
      )
    );
  }
